...
def _regular_payload(*, tp_size: int = 1, pp_size: int = 1) -> dict[str, Any]:
    return dict(tp_size=tp_size, pp_size=pp_size, dp_size=1, disaggregation_mode="null")


def _prefill_payload(*, tp_size: int = 1, bootstrap_port: int = 12090) -> dict[str, Any]:
    return dict(tp_size=tp_size, pp_size=1, disaggregation_mode="prefill", disaggregation_bootstrap_port=bootstrap_port)


def _decode_payload(*, tp_size: int = 1) -> dict[str, Any]:
    return dict(tp_size=tp_size, pp_size=1, disaggregation_mode="decode")
...
class TestDiscoverExternalEngine:
    async def test_num_gpus_is_tp_times_pp(self, monkeypatch):
        """sglang's /server_info reports the parallel sizes, and tp x pp is the engine's world."""
        _install_payloads(monkeypatch, {"http://host1:8000": _regular_payload(tp_size=4, pp_size=2)})
    ...
    async def test_a_prefill_engine_reports_its_type_and_bootstrap_port(self, monkeypatch):
        ...
        _install_payloads(monkeypatch, 
            {
                "http://host1:8000": dict(
                    tp_size=2, pp_size=1, disaggregation_mode="prefill", disaggregation_bootstrap_port="12090"
                )
            }
        )
    ...
    async def test_a_decode_engine_reports_its_type_without_a_bootstrap_port(self, monkeypatch):
        ...
        _install_payloads(monkeypatch, {"http://host1:8000": dict(tp_size=4, pp_size=1, disaggregation_mode="decode")})
...
class TestStaticInferenceEngineWorkerProvider:
    ...
    async def test_each_url_becomes_one_cell_with_the_discovered_meta(self, monkeypatch):
        ...
        provider = await self._make_provider(monkeypatch, args, {"http://host1:8000": _regular_payload(tp_size=2)})
    ...
    async def test_gpu_offsets_accumulate_linearly_in_listed_order(self, monkeypatch):
        ...
        payloads = {
            "http://host1:8000": _regular_payload(tp_size=2),
            "http://host2:8000": _regular_payload(tp_size=4),
            "http://host3:8000": _regular_payload(tp_size=1),
        }
    ...
    async def test_a_fleet_with_a_different_gpu_total_is_rejected(self, monkeypatch):
        ...
        payloads = {
            "http://host1:8000": _regular_payload(tp_size=4),
            "http://host2:8000": _regular_payload(tp_size=2),
        }
    ...
    async def test_a_declared_pd_fleet_carries_roles_and_bootstrap_ports(self, monkeypatch):
        ...
        payloads = {
            "http://host1:8000": _prefill_payload(tp_size=2, bootstrap_port=12090),
            "http://host2:8000": _decode_payload(tp_size=4),
        }
    ...
    async def test_the_cell_info_satisfies_the_server_cell_meta_contract(self, monkeypatch):
        ...
        payload = _decode_payload(tp_size=2)
