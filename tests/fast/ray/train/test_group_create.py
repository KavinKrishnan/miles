...
async def _create_controller(provider: _RecordingWorkerProvider, *, num_cells: int) -> TrainerController:
...
class TestCreate:
    async def test_create_subscribes_reconcile_to_the_trainer_spec(self, provider):
        ...
        controller = await _create_controller(provider, num_cells=2)
        try:
    ...
    async def test_dispose_stops_the_watch_loop(self, provider):
        ...
        controller = await _create_controller(provider, num_cells=1)
