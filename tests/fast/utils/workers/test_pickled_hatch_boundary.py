...
def _pickled_parameters_in(source: str, *, label: str) -> set[tuple[str, str, str]]:
        ...
        for method in ast.walk(class_node):
            ...
            for parameter in [*arguments.posonlyargs, *arguments.args, *arguments.kwonlyargs]:
                if _is_pickled(parameter.annotation, aliases):
                    found.add((label, f"{class_node.name}.{method.name}", parameter.arg))
    return found


def _pickled_parameters_of(path: Path) -> set[tuple[str, str, str]]:
    return _pickled_parameters_in(path.read_text(), label=str(path.relative_to(REPO_ROOT)))


@functools.cache
def _all_pickled_parameters() -> frozenset[tuple[str, str, str]]:
    return frozenset(
        entry for path in shipped_modules(exclude_dirs=EXCLUDED_DIRS) for entry in _pickled_parameters_of(path)
    )


class TestThePickledHatchStaysWhitelisted:
    def test_no_parameter_uses_the_hatch_without_being_listed(self):
        """The hatch punches a hole in the strict wire typing, so every user of it is reviewed here."""
        unlisted = sorted(_all_pickled_parameters() - PICKLED_PARAMETERS)
    ...
    def test_the_list_names_no_parameter_that_no_longer_uses_it(self):
        ...
        stale = sorted(PICKLED_PARAMETERS - _all_pickled_parameters())
...
class TestTheShapesThatUsedToCarryTheHatchThrough:
    def test_a_union_is_seen(self):
        ...
        assert _pickled_parameters_in(source, label="w.py") == {("w.py", "W.go", "args")}
    ...
    def test_a_wire_typed_parameter_is_not_reported(self):
        ...
        assert _pickled_parameters_in(source, label="w.py") == set()
