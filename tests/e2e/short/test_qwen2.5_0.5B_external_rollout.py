...
def _external_engines_launch_cmd() -> str:
    launches = " ".join(
        f"CUDA_VISIBLE_DEVICES={gpu} nohup python3 -m sglang.launch_server "
        ...
        for gpu, port in zip(ENGINE_GPUS, ENGINE_PORTS, strict=True)
    )
...
def execute():
    ...
    U.execute_train(
        ...
        prepare_cmd={"trainer": _external_engines_launch_cmd()},
