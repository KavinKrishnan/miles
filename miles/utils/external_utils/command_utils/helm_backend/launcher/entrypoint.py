...
def execute_train(*, request: ExecuteTrainRequest, config: ExecuteTrainConfig) -> None:
    ...
    if installed_manifest is None:
        ...
        _assert_upgrade_only_resizes(
            ...
            force=config.force,
            rebuilt_object_names=hot_restart.rebuilt_object_names,
        )
...
def _parse_train_args(request: ExecuteTrainRequest, *, run_id: str) -> Any:
    with override_argv(_compute_base_argv(request, run_id=run_id)):
        return parse_args()


def _compute_pod_argv(
    request: ExecuteTrainRequest,
    *,
    run_id: str,
    release: str,
    namespace: str,
    mooncake_plan: MooncakePlan | None,
) -> list[str]:
    return MooncakeInfo.with_cluster_master(
        _compute_base_argv(request, run_id=run_id),
        plan=mooncake_plan,
...
def _assert_upgrade_only_resizes(
    *,
    ...
    force: bool,
    rebuilt_object_names: frozenset[str],
) -> None:
    ...
    diff = manifest_diff.diff_manifests(
        before=installed_manifest, after=proposed_manifest, rebuilt_object_names=rebuilt_object_names
    )
    ...
    scope = (
        "more than its size"
        if not rebuilt_object_names
        else f"more than the objects this hot restart replaces ({sorted(rebuilt_object_names)})"
    )
    message = (
...
def _belongs_to_run(release: str, *, run_id: str) -> bool:
    ...
    return release == own or release.startswith(f"{own}-")
